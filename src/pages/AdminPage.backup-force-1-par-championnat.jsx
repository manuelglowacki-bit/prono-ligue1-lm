import React, { useEffect, useMemo, useRef, useState } from "react";
import * as XLSX from "xlsx";
import { parseExcelWorkbookToJournees } from "../utils/excelJournees.js";
import { supabase } from "../lib/supabaseClient";

const STORAGE_KEY = "admin_pronos_manager_v3";
const PUBLIC_JOURNEES_KEY = "admin_journees";
const BONUS_RANDOM_START_KEY = "admin_bonus_random_start_datetime";
const BONUS_RANDOM_END_KEY = "admin_bonus_random_end_datetime";

const DEFAULT_JOURNEES = [
  {
    id: "j1",
    number: 1,
    title: "J1",
    locked: false,
    lockAt: "",
    matches: [
      {
        id: "j1-m1",
        home: "RC Lens",
        away: "OL",
        date: "2026-08-22",
        time: "20:45",
        status: "Ouvert",
        homeScore: "",
        awayScore: "",
        adminValidated: false
      },
      {
        id: "j1-m2",
        home: "PSG",
        away: "Nantes",
        date: "2026-08-23",
        time: "17:00",
        status: "Ouvert",
        homeScore: "",
        awayScore: "",
        adminValidated: false
      },
      {
        id: "j1-m3",
        home: "OM",
        away: "Rennes",
        date: "2026-08-23",
        time: "21:00",
        status: "Ouvert",
        homeScore: "",
        awayScore: "",
        adminValidated: false
      }
    ],
    bonus: [
      {
        id: "j1-b1",
        league: "Premier League",
        home: "Liverpool",
        away: "Chelsea",
        date: "2026-08-22",
        time: "18:30",
        homeScore: "",
        awayScore: "",
        adminValidated: false
      },
      {
        id: "j1-b2",
        league: "Bundesliga",
        home: "Bayern",
        away: "Dortmund",
        date: "2026-08-22",
        time: "18:30",
        homeScore: "",
        awayScore: "",
        adminValidated: false
      },
      {
        id: "j1-b3",
        league: "Serie A",
        home: "Milan",
        away: "Naples",
        date: "2026-08-23",
        time: "20:45",
        homeScore: "",
        awayScore: "",
        adminValidated: false
      }
    ]
  }
];

function loadJson(key, fallback) {
  try {
    const raw = localStorage.getItem(key);
    return raw ? JSON.parse(raw) : fallback;
  } catch {
    return fallback;
  }
}

function savePublicJourneesLocal(journees) {
  localStorage.setItem(PUBLIC_JOURNEES_KEY, JSON.stringify(journees));
  localStorage.setItem(STORAGE_KEY, JSON.stringify(journees));
  window.dispatchEvent(new Event("storage"));
  window.dispatchEvent(new CustomEvent("calendar-updated"));
  window.dispatchEvent(new CustomEvent("results-updated"));
}

async function savePublicJourneesOnline(journees) {
  savePublicJourneesLocal(journees);

  const { error } = await supabase
    .from("app_settings")
    .upsert(
      {
        setting_key: PUBLIC_JOURNEES_KEY,
        setting_value: journees,
        updated_at: new Date().toISOString()
      },
      { onConflict: "setting_key" }
    );

  if (error) {
    throw error;
  }
}

function savePublicJournees(journees) {
  savePublicJourneesLocal(journees);
}

function toDateInputValue(value) {
  const raw = String(value || "").trim();

  if (!raw) return "";

  if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) {
    return raw;
  }

  const fr = raw.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})$/);

  if (fr) {
    const day = fr[1].padStart(2, "0");
    const month = fr[2].padStart(2, "0");
    const year = fr[3];
    return `${year}-${month}-${day}`;
  }

  return "";
}

function toTimeInputValue(value) {
  const raw = String(value || "").trim();

  if (!raw) return "";

  const match = raw.match(/(\d{1,2})[:hH](\d{2})/);

  if (match) {
    const hour = match[1].padStart(2, "0");
    const minute = match[2].padStart(2, "0");
    return `${hour}:${minute}`;
  }

  return "";
}

function normalizeJournees(data) {
  if (!Array.isArray(data) || data.length === 0) return DEFAULT_JOURNEES;

  return data.map((j, index) => {
    const journeyId = j.id || `j${index + 1}`;

    return {
      id: journeyId,
      number: j.number || index + 1,
      title: j.title || `J${index + 1}`,
      locked: Boolean(j.locked),
      lockAt: j.lockAt || "",
      matches: Array.isArray(j.matches)
        ? j.matches.map((m, i) => ({
            id: m.id || `${journeyId}-m${i + 1}`,
            home: m.home || "",
            away: m.away || "",
            date: toDateInputValue(m.date) || "",
            time: toTimeInputValue(m.time) || "",
            status: m.status || "Ouvert",
            homeScore: m.homeScore ?? "",
            awayScore: m.awayScore ?? "",
            adminValidated: Boolean(m.adminValidated),
            validated: Boolean(m.validated),
            isValidated: Boolean(m.isValidated),
            result: m.result || m.resultat || "",
            resultat: m.resultat || m.result || ""
          }))
        : [],
      bonus: Array.isArray(j.bonus)
        ? j.bonus.slice(0, 4).map((b, i) => ({
            id: b.id || `${journeyId}-b${i + 1}`,
            league: b.league || "",
            home: b.home || "",
            away: b.away || "",
            date: toDateInputValue(b.date) || "",
            time: toTimeInputValue(b.time) || "",
            homeScore: b.homeScore ?? "",
            awayScore: b.awayScore ?? "",
            adminValidated: Boolean(b.adminValidated),
            validated: Boolean(b.validated),
            isValidated: Boolean(b.isValidated),
            result: b.result || b.resultat || "",
            resultat: b.resultat || b.result || ""
          }))
        : []
    };
  });
}

function makeEmptyBonus(journeyId, index) {
  return {
    id: `${journeyId}-b${index + 1}`,
    league: "",
    home: "",
    away: "",
    date: "",
    time: "",
    homeScore: "",
    awayScore: "",
    adminValidated: false
  };
}

function ensureThreeBonus(journee) {
  const bonus = Array.isArray(journee.bonus) ? [...journee.bonus] : [];

  while (bonus.length < 4) {
    bonus.push(makeEmptyBonus(journee.id, bonus.length));
  }

  return {
    ...journee,
    lockAt: journee.lockAt || "",
    bonus: bonus.slice(0, 4)
  };
}

export default function AdminPage() {
  const fileInputRef = useRef(null);
  const bonusFileInputRef = useRef(null);

  const [journees, setJournees] = useState(() => {
    const saved =
      loadJson(PUBLIC_JOURNEES_KEY, null) ||
      loadJson(STORAGE_KEY, null) ||
      DEFAULT_JOURNEES;

    return normalizeJournees(saved).map(ensureThreeBonus);
  });

  const [selectedJourneeId, setSelectedJourneeId] = useState(() => {
    const saved =
      loadJson(PUBLIC_JOURNEES_KEY, null) ||
      loadJson(STORAGE_KEY, null) ||
      DEFAULT_JOURNEES;

    return normalizeJournees(saved)[0]?.id || "j1";
  });

  const [message, setMessage] = useState("");
  const [bonusRandomStartDate, setBonusRandomStartDate] = useState(() => localStorage.getItem(BONUS_RANDOM_START_KEY) || "");
  const [bonusRandomEndDate, setBonusRandomEndDate] = useState(() => localStorage.getItem(BONUS_RANDOM_END_KEY) || "");

  const selectedJournee = useMemo(() => {
    return (
      journees.find((j) => j.id === selectedJourneeId) ||
      journees[0] ||
      DEFAULT_JOURNEES[0]
    );
  }, [journees, selectedJourneeId]);

  // load-online-admin-journees
  useEffect(() => {
    let mounted = true;

    async function loadOnlineJournees() {
      try {
        const { data, error } = await supabase
          .from("app_settings")
          .select("setting_value")
          .eq("setting_key", PUBLIC_JOURNEES_KEY)
          .maybeSingle();

        if (error) throw error;

        const online = data?.setting_value;

        if (mounted && Array.isArray(online) && online.length) {
          const normalized = normalizeJournees(online).map(ensureThreeBonus);
          setJournees(normalized);
          setSelectedJourneeId(normalized[0]?.id || "j1");
          savePublicJourneesLocal(normalized);
        }
      } catch (error) {
        console.error("Erreur chargement calendrier cloud", error);
      }
    }

    loadOnlineJournees();

    return () => {
      mounted = false;
    };
  }, []);

  useEffect(() => {
    savePublicJourneesLocal(journees);
  }, [journees]);

  useEffect(() => {
    localStorage.setItem(BONUS_RANDOM_START_KEY, bonusRandomStartDate || "");
    localStorage.setItem(BONUS_RANDOM_END_KEY, bonusRandomEndDate || "");
  }, [bonusRandomStartDate, bonusRandomEndDate]);

  useEffect(() => {
    localStorage.setItem(BONUS_RANDOM_START_KEY, bonusRandomStartDate || "");
    localStorage.setItem(BONUS_RANDOM_END_KEY, bonusRandomEndDate || "");
  }, [bonusRandomStartDate, bonusRandomEndDate]);

  function updateSelectedJournee(patch) {
    setJournees((prev) => {
      const next = prev.map((j) =>
        j.id === selectedJournee.id
          ? ensureThreeBonus({ ...j, ...patch })
          : j
      );

      savePublicJourneesLocal(next);

      savePublicJourneesOnline(next).catch((error) => {
        console.error("Erreur sauvegarde cloud admin", error);
      });

      return next;
    });
  }

  function updateMatch(matchId, patch) {
    const nextMatches = selectedJournee.matches.map((match) =>
      match.id === matchId ? { ...match, ...patch, adminValidated: patch.adminValidated ?? false } : match
    );

    updateSelectedJournee({ matches: nextMatches });
  }

  function updateBonus(bonusId, patch) {
    const currentBonus = ensureThreeBonus(selectedJournee).bonus;

    const nextBonus = currentBonus.map((bonus) =>
      bonus.id === bonusId ? { ...bonus, ...patch, adminValidated: patch.adminValidated ?? false } : bonus
    );

    updateSelectedJournee({ bonus: nextBonus });
  }

  function adminScoreIsFilled(value) {
    return value !== "" && value !== null && value !== undefined;
  }

  function adminToScore(value) {
    if (!adminScoreIsFilled(value)) return "";
    const number = Number(value);
    return Number.isNaN(number) ? "" : number;
  }

  function adminResultFromScore(home, away) {
    if (home > away) return "1";
    if (home < away) return "2";
    return "N";
  }

  function buildAdminValidatedPatch(match) {
    const home = adminToScore(match.homeScore);
    const away = adminToScore(match.awayScore);

    if (!adminScoreIsFilled(home) || !adminScoreIsFilled(away)) {
      return null;
    }

    const resultat = adminResultFromScore(home, away);

    return {
      homeScore: home,
      awayScore: away,
      scoreDomicile: home,
      scoreExterieur: away,
      score: home + "-" + away,
      resultat,
      result: resultat,
      resultatFinal: resultat,
      status: "FINISHED",
      statut: "FINISHED",
      statutLong: "Résultat validé",
      adminValidated: true,
      validated: true,
      isValidated: true,
      resultValidatedAt: new Date().toISOString()
    };
  }

  function validateAdminMatch(matchId) {
    const regularMatch = selectedJournee.matches.find((item) => item.id === matchId);

    if (regularMatch) {
      const patch = buildAdminValidatedPatch(regularMatch);

      if (!patch) {
        alert("Mets le resultat final avant de valider.");
        return;
      }

      const nextMatches = selectedJournee.matches.map((item) =>
        item.id === matchId ? { ...item, ...patch } : item
      );

      updateSelectedJournee({ matches: nextMatches });
      setMessage("Resultat du match valide.");
      return;
    }

    const currentBonus = ensureThreeBonus(selectedJournee).bonus;
    const bonusMatch = currentBonus.find((item) => item.id === matchId);

    if (bonusMatch) {
      const patch = buildAdminValidatedPatch(bonusMatch);

      if (!patch) {
        alert("Mets le resultat final bonus avant de valider.");
        return;
      }

      const nextBonus = currentBonus.map((item) =>
        item.id === matchId ? { ...item, ...patch } : item
      );

      updateSelectedJournee({ bonus: nextBonus });
      setMessage("Resultat bonus valide.");
    }
  }

  function toggleMatchStatus(matchId) {
    const match = selectedJournee.matches.find((m) => m.id === matchId);
    const nextStatus = match?.status === "Bloque" ? "Ouvert" : "Bloque";

    updateMatch(matchId, { status: nextStatus });

    setMessage(nextStatus === "Bloque" ? "Match bloque." : "Match ouvert.");
  }

  function addMatch() {
    const number = selectedJournee.matches.length + 1;

    const newMatch = {
      id: `${selectedJournee.id}-m${number}`,
      home: "",
      away: "",
      date: "",
      time: "",
      status: "Ouvert",
      homeScore: "",
      awayScore: "",
      adminValidated: false
    };

    updateSelectedJournee({
      matches: [...selectedJournee.matches, newMatch]
    });

    setMessage("Nouveau match ajoute.");
  }

  function toggleLock() {
    const locked = !selectedJournee.locked;

    updateSelectedJournee({ locked });

    setMessage(locked ? "Journee fermee." : "Journee rouverte.");
  }

  function saveAll() {
    savePublicJournees(journees);
    localStorage.setItem(BONUS_RANDOM_START_KEY, bonusRandomStartDate || "");
    localStorage.setItem(BONUS_RANDOM_END_KEY, bonusRandomEndDate || "");
    setMessage("Sauvegarde OK. Dates bonus, matchs et resultats enregistres.");
  }

  function previousJournee() {
    const index = journees.findIndex((j) => j.id === selectedJournee.id);
    const previous = journees[Math.max(0, index - 1)];
    if (previous) setSelectedJourneeId(previous.id);
  }

  function nextJournee() {
    const index = journees.findIndex((j) => j.id === selectedJournee.id);
    const next = journees[Math.min(journees.length - 1, index + 1)];
    if (next) setSelectedJourneeId(next.id);
  }

  function addJournee() {
    const number = journees.length + 1;

    const newJournee = ensureThreeBonus({
      id: `j${number}`,
      number,
      title: `J${number}`,
      locked: false,
      lockAt: "",
      matches: [],
      bonus: []
    });

    setJournees((prev) => [...prev, newJournee]);
    setSelectedJourneeId(newJournee.id);
    setMessage(`J${number} ajoutee.`);
  }

  function handleExcelImport(event) {
    const file = event.target.files?.[0];
    if (!file) return;

    const reader = new FileReader();

    reader.onload = (e) => {
      try {
        const data = new Uint8Array(e.target.result);

        const workbook = XLSX.read(data, {
          type: "array",
          cellDates: true
        });

        const imported = parseExcelWorkbookToJournees(workbook)
          .map((j) => ({
            ...j,
            locked: false,
            lockAt: "",
            matches: (j.matches || []).map((m) => ({
              ...m,
              date: toDateInputValue(m.date) || "",
              time: toTimeInputValue(m.time) || "",
              status: m.status || "Ouvert",
              homeScore: m.homeScore || "",
              awayScore: m.awayScore || "",
              adminValidated: false
            })),
            bonus: (j.bonus || []).map((b) => ({
              ...b,
              date: toDateInputValue(b.date) || "",
              time: toTimeInputValue(b.time) || "",
              homeScore: b.homeScore || "",
              awayScore: b.awayScore || "",
              adminValidated: false
            }))
          }))
          .map(ensureThreeBonus);

        if (!imported.length) {
          setMessage("Import impossible : aucune ligne valide trouvee.");
          return;
        }

        setJournees(imported);
        setSelectedJourneeId(imported[0].id);
        savePublicJourneesOnline(imported).catch((error) => {
          console.error("Erreur sauvegarde cloud import Excel", error);
        });

        setMessage(`${imported.length} journee(s) importee(s) depuis Excel.`);
      } catch (error) {
        console.error(error);
        setMessage("Erreur import Excel.");
      } finally {
        event.target.value = "";
      }
    };

    reader.readAsArrayBuffer(file);
  }


  function normalizeBonusHeader(value) {
    return String(value || "")
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/[^a-z0-9]/g, "");
  }

  function getBonusCell(row, names) {
    const wanted = names.map(normalizeBonusHeader);

    for (const key of Object.keys(row)) {
      if (wanted.includes(normalizeBonusHeader(key))) {
        return row[key];
      }
    }

    return "";
  }

  function normalizeBonusLeague(value) {
    const raw = String(value || "").trim();
    const key = raw
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");

    if (key.includes("premier") || key.includes("angleterre") || key.includes("england")) {
      return "Premier League";
    }

    if (key.includes("liga") || key.includes("espagne") || key.includes("spain")) {
      return "Liga";
    }

    if (key.includes("serie") || key.includes("italie") || key.includes("italy")) {
      return "Serie A";
    }

    if (key.includes("bundesliga") || key.includes("allemagne") || key.includes("germany")) {
      return "Bundesliga";
    }

    return raw;
  }

  function isAllowedBonusLeague(value) {
    const league = normalizeBonusLeague(value);
    return ["Premier League", "Liga", "Serie A", "Bundesliga"].includes(league);
  }

  function bonusDateInputValue(value) {
    if (!value) return "";

    if (value instanceof Date && !Number.isNaN(value.getTime())) {
      const year = value.getFullYear();
      const month = String(value.getMonth() + 1).padStart(2, "0");
      const day = String(value.getDate()).padStart(2, "0");
      return `${year}-${month}-${day}`;
    }

    if (typeof value === "number") {
      const parsed = XLSX.SSF.parse_date_code(value);
      if (parsed) {
        const year = parsed.y;
        const month = String(parsed.m).padStart(2, "0");
        const day = String(parsed.d).padStart(2, "0");
        return `${year}-${month}-${day}`;
      }
    }

    return toDateInputValue(value);
  }

  function bonusTimeInputValue(value) {
    if (!value) return "";

    if (value instanceof Date && !Number.isNaN(value.getTime())) {
      const hour = String(value.getHours()).padStart(2, "0");
      const minute = String(value.getMinutes()).padStart(2, "0");
      return `${hour}:${minute}`;
    }

    if (typeof value === "number") {
      const parsed = XLSX.SSF.parse_date_code(value);
      if (parsed) {
        const hour = String(parsed.H || 0).padStart(2, "0");
        const minute = String(parsed.M || 0).padStart(2, "0");
        return `${hour}:${minute}`;
      }
    }

    return toTimeInputValue(value);
  }

  function rowMatchesSelectedJournee(row) {
    const value = getBonusCell(row, [
      "Journée",
      "Journee",
      "J",
      "N° journée",
      "Numero journee",
      "Round"
    ]);

    if (!value) return true;

    const raw = String(value).toLowerCase().replace(/\s+/g, "");
    const currentNumber = String(selectedJournee.number || "").toLowerCase();
    const currentTitle = String(selectedJournee.title || "").toLowerCase().replace(/\s+/g, "");

    return (
      raw === currentNumber ||
      raw === currentTitle ||
      raw === `j${currentNumber}`
    );
  }
  function hasBonusDateRange() {
    return Boolean(bonusRandomStartDate || bonusRandomEndDate);
  }

  function isBonusDateInRange(dateValue, timeValue = "") {
    const date = String(dateValue || "").slice(0, 10);
    const time = String(timeValue || "00:00").slice(0, 5);
    const dateTime = date ? `${date}T${time}` : "";

    if (!dateTime) return false;

    if (bonusRandomStartDate && dateTime < bonusRandomStartDate) {
      return false;
    }

    if (bonusRandomEndDate && dateTime > bonusRandomEndDate) {
      return false;
    }

    return true;
  }



  function shuffleBonusMatches(list) {
    const arr = [...list];

    for (let i = arr.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      const temp = arr[i];
      arr[i] = arr[j];
      arr[j] = temp;
    }

    return arr;
  }

  function getBonusLeagueBucket(value) {
  const text = String(value || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "");

  if (text.includes("premier") || text.includes("angleterre") || text.includes("england")) {
    return "Premier League";
  }

  if (text.includes("liga") || text.includes("espagne") || text.includes("spain")) {
    return "Liga";
  }

  if (text.includes("serie") || text.includes("italie") || text.includes("italy")) {
    return "Serie A";
  }

  if (text.includes("bundes")) {
    return "Bundesliga";
  }

  return normalizeBonusLeague(value);
}

function pickRandomBonusMatches(matches, count = 4) {
  const wantedLeagues = ["Premier League", "Liga", "Serie A", "Bundesliga"];

  const clean = shuffleBonusMatches(
    matches.filter((match) => isAllowedBonusLeague(match.league))
  );

  const selected = [];
  const usedKeys = new Set();

  function matchKey(match) {
    return [
      match.league || "",
      match.home || match.domicile || "",
      match.away || match.exterieur || "",
      match.date || "",
      match.time || ""
    ].join("|");
  }

  wantedLeagues.forEach((wantedLeague) => {
    const candidates = clean.filter((match) => {
      const leagueBucket = getBonusLeagueBucket(match.league);
      const key = matchKey(match);

      return leagueBucket === wantedLeague && !usedKeys.has(key);
    });

    if (candidates.length) {
      const picked = candidates[Math.floor(Math.random() * candidates.length)];
      selected.push({
        ...picked,
        league: wantedLeague,
        championnat: wantedLeague
      });
      usedKeys.add(matchKey(picked));
    }
  });

  return selected.slice(0, count);
}

function handleBonusExcelImport(event) {
    const file = event.target.files?.[0];
    if (!file) return;

    const reader = new FileReader();

    reader.onload = (e) => {
      try {
        const data = new Uint8Array(e.target.result);

        const workbook = XLSX.read(data, {
          type: "array",
          cellDates: true
        });

        const rows = workbook.SheetNames.flatMap((sheetName) => {
          const sheet = workbook.Sheets[sheetName];
          return XLSX.utils.sheet_to_json(sheet, { defval: "" });
        });

        const availableBonusRows = rows
          .filter((row) => {
            if (!hasBonusDateRange()) {
              return rowMatchesSelectedJournee(row);
            }

            const dateValue = getBonusCell(row, ["Date", "Jour", "Match date"]);
            const timeValue = getBonusCell(row, ["Heure", "Horaire", "Time", "Kickoff"]);
            return isBonusDateInRange(bonusDateInputValue(dateValue), bonusTimeInputValue(timeValue));
          })
          .map((row) => {
            const leagueRaw = getBonusCell(row, [
              "Championnat",
              "Competition",
              "Compétition",
              "League",
              "Ligue"
            ]);

            const home = getBonusCell(row, [
              "Domicile",
              "Equipe domicile",
              "Équipe domicile",
              "Home",
              "Home Team"
            ]);

            const away = getBonusCell(row, [
              "Extérieur",
              "Exterieur",
              "Equipe exterieur",
              "Équipe extérieur",
              "Away",
              "Away Team"
            ]);

            const date = getBonusCell(row, ["Date", "Jour", "Match date"]);
            const time = getBonusCell(row, ["Heure", "Horaire", "Time", "Kickoff"]);

            return {
              league: normalizeBonusLeague(leagueRaw),
              home: String(home || "").trim(),
              away: String(away || "").trim(),
              date: bonusDateInputValue(date),
              time: bonusTimeInputValue(time)
            };
          })
          .filter((match) =>
            isAllowedBonusLeague(match.league) &&
            match.home &&
            match.away
          );

        const bonusRows = pickRandomBonusMatches(availableBonusRows, 3);

        if (!bonusRows.length) {
          setMessage("Import bonus impossible : aucun match trouve dans la periode choisie.");
          return;
        }

        const currentBonus = ensureThreeBonus(selectedJournee).bonus;

        const nextBonus = [0, 1, 2, 3].map((index) => {
          const imported = bonusRows[index];
          const previous = currentBonus[index] || makeEmptyBonus(selectedJournee.id, index);

          if (!imported) {
            return previous;
          }

          return {
            ...previous,
            league: imported.league,
            home: imported.home,
            away: imported.away,
            date: imported.date,
            time: imported.time,
            homeScore: "",
            awayScore: "",
            adminValidated: false,
            validated: false,
            isValidated: false
          };
        });

        updateSelectedJournee({
          bonus: nextBonus
        });

        setMessage(`${bonusRows.length} match(s) bonus tiré(s) au hasard entre les dates choisies.`);
      } catch (error) {
        console.error(error);
        setMessage("Erreur import bonus Excel.");
      } finally {
        event.target.value = "";
      }
    };

    reader.readAsArrayBuffer(file);
  }


  const matches = selectedJournee.matches || [];
  const bonus = ensureThreeBonus(selectedJournee).bonus;

  return (
    <div className="admin-pronos-page">
      <style>{`
        .admin-pronos-page {
          color: #ffffff;
          padding: 4px;
        }

        .admin-pronos-top {
          display: grid;
          grid-template-columns: 280px 1fr;
          gap: 32px;
          margin-bottom: 28px;
          padding: 36px;
          border-radius: 28px;
          background:
            radial-gradient(circle at top left, rgba(186,255,0,.18), transparent 34%),
            linear-gradient(135deg, rgba(15,23,42,.98), rgba(29,78,216,.35));
          border: 1px solid rgba(255,255,255,.10);
        }

        .admin-pronos-title h1 {
          margin: 0;
          font-size: 38px;
          line-height: .98;
          font-weight: 950;
          letter-spacing: -.04em;
        }

        .admin-pronos-title p {
          margin: 12px 0 0;
          color: #f8fafc;
          font-weight: 850;
          line-height: 1.35;
        }

        .admin-pronos-actions {
          display: flex;
          gap: 12px;
          flex-wrap: wrap;
          justify-content: flex-end;
          align-items: flex-start;
        }

        .admin-btn,
        .admin-select {
          border: 1px solid rgba(255,255,255,.14);
          background: rgba(255,255,255,.08);
          color: #f8fafc;
          border-radius: 16px;
          padding: 16px 22px;
          font-weight: 950;
          cursor: pointer;
          outline: none;
        }

        .admin-select {
          background: #111827;
          min-width: 130px;
        }

        .admin-btn.green {
          background: #baff00;
          border-color: #baff00;
          color: #07111f;
        }

        .admin-btn.gold {
          background: #ffd21f;
          border-color: #ffd21f;
          color: #07111f;
        }

        .admin-message {
          grid-column: 1 / -1;
          margin-top: 18px;
          padding: 18px 20px;
          border-radius: 18px;
          background: rgba(186,255,0,.08);
          border: 1px solid rgba(186,255,0,.20);
          color: #d9ff66;
          font-weight: 950;
        }

        .section-head {
          display: flex;
          justify-content: space-between;
          align-items: center;
          margin: 28px 0 14px;
        }

        .section-head h2 {
          margin: 0;
          font-size: 26px;
          font-weight: 950;
        }

        .section-head span {
          color: #b6c2d9;
          font-weight: 900;
        }

        .admin-match-grid {
          display: grid;
          grid-template-columns: repeat(3, minmax(0, 1fr));
          gap: 16px;
        }

        .admin-match-card {
          background: linear-gradient(180deg, rgba(20,32,51,.96), rgba(11,18,32,.98));
          border: 1px solid rgba(255,255,255,.12);
          border-radius: 22px;
          padding: 16px;
          box-shadow: 0 16px 38px rgba(0,0,0,.28);
        }

        .admin-match-card.blocked {
          border-color: rgba(239,68,68,.38);
          opacity: .92;
        }

        .admin-match-card.bonus {
          border-color: rgba(34,197,94,.32);
        }

        .admin-match-top {
          display: flex;
          justify-content: space-between;
          align-items: center;
          gap: 10px;
          margin-bottom: 14px;
        }

        .admin-number {
          width: 50px;
          height: 50px;
          border-radius: 16px;
          background: rgba(186,255,0,.16);
          color: #baff00;
          display: grid;
          place-items: center;
          font-size: 24px;
          font-weight: 950;
        }

        .admin-status-btn {
          border-radius: 999px;
          padding: 10px 14px;
          border: 1px solid rgba(34,197,94,.30);
          background: rgba(34,197,94,.14);
          color: #86efac;
          font-size: 13px;
          font-weight: 950;
          cursor: pointer;
        }

        .admin-status-btn.blocked {
          background: rgba(239,68,68,.14);
          color: #fca5a5;
          border-color: rgba(239,68,68,.30);
        }

        .admin-pill {
          border-radius: 999px;
          padding: 8px 12px;
          background: rgba(34,197,94,.14);
          color: #86efac;
          border: 1px solid rgba(34,197,94,.30);
          font-size: 12px;
          font-weight: 950;
        }

        .admin-fields {
          display: grid;
          gap: 10px;
        }

        .admin-fields label {
          display: grid;
          gap: 6px;
          color: #9fb0ca;
          font-size: 12px;
          font-weight: 950;
          text-transform: uppercase;
          letter-spacing: .08em;
        }

        .admin-input {
          width: 100%;
          background: #111827;
          color: #f8fafc;
          border: 1px solid rgba(255,255,255,.14);
          border-radius: 14px;
          padding: 12px 12px;
          font-weight: 900;
          outline: none;
        }

        .admin-input:focus {
          border-color: rgba(186,255,0,.55);
        }

        .admin-two {
          display: grid;
          grid-template-columns: 1fr 1fr;
          gap: 10px;
        }

        .score-row {
          display: flex;
          align-items: center;
          gap: 8px;
        }

        .score-row input {
          text-align: center;
        }

        .admin-card-actions {
          display: flex;
          gap: 8px;
          margin-top: 12px;
        }

        .admin-card-actions button {
          flex: 1;
          border-radius: 999px;
          padding: 12px 14px;
          border: 1px solid rgba(255,255,255,.14);
          background: rgba(255,255,255,.08);
          color: #f8fafc;
          font-weight: 950;
          cursor: pointer;
        }

        .admin-card-actions button.red {
          background: rgba(239,68,68,.18);
          color: #fca5a5;
          border-color: rgba(239,68,68,.30);
        }

        .admin-validate-btn.validated {
          background: rgba(34,197,94,.22) !important;
          border-color: rgba(34,197,94,.65) !important;
          color: #86efac !important;
        }

        @media (max-width: 1200px) {
          .admin-pronos-top {
            grid-template-columns: 1fr;
          }

          .admin-pronos-actions {
            justify-content: flex-start;
          }

          .admin-match-grid {
            grid-template-columns: repeat(2, minmax(0, 1fr));
          }
        }

        @media (max-width: 760px) {
          .admin-pronos-top {
            padding: 24px;
          }

          .admin-pronos-actions,
          .admin-btn,
          .admin-select {
            width: 100%;
          }

          .admin-match-grid {
            grid-template-columns: 1fr;
          }

          .admin-pronos-title h1 {
            font-size: 32px;
          }
        }
      `}</style>

      <input
        ref={fileInputRef}
        type="file"
        accept=".xlsx,.xls,.csv"
        style={{ display: "none" }}
        onChange={handleExcelImport}
      />

      <input
        ref={bonusFileInputRef}
        type="file"
        accept=".xlsx,.xls,.csv"
        style={{ display: "none" }}
        onChange={handleBonusExcelImport}
      />

      <div className="admin-pronos-top">
        <div className="admin-pronos-title">
          <h1>Admin pronostics</h1>
          <p>Meme presentation que la page Pronos, mais ici tu geres les journees, les matchs et les bonus.</p>
        </div>

        <div className="admin-pronos-actions">
          <button className="admin-btn green" type="button" onClick={() => fileInputRef.current?.click()}>
            Importer Excel
          </button>

                    <label className="admin-bonus-date-filter">
            <span>Date début bonus</span>
            <input
              type="datetime-local"
              value={bonusRandomStartDate}
              onChange={(event) => setBonusRandomStartDate(event.target.value)}
            />
          </label>

          <label className="admin-bonus-date-filter">
            <span>Date fin bonus</span>
            <input
              type="datetime-local"
              value={bonusRandomEndDate}
              onChange={(event) => setBonusRandomEndDate(event.target.value)}
            />
          </label>

<button className="admin-btn green" type="button" onClick={() => bonusFileInputRef.current?.click()}>
            Importer bonus Excel aléatoire
          </button>

          <button className="admin-btn" type="button" onClick={previousJournee}>
            Journee avant
          </button>

          <select
            className="admin-select"
            value={selectedJournee.id}
            onChange={(e) => setSelectedJourneeId(e.target.value)}
          >
            {journees.map((j) => (
              <option key={j.id} value={j.id}>
                {j.title}
              </option>
            ))}
          </select>

          <button className="admin-btn" type="button" onClick={nextJournee}>
            Journee apres
          </button>

          <button className="admin-btn" type="button" onClick={addJournee}>
            Ajouter journee
          </button>

          <button className="admin-btn gold" type="button" onClick={toggleLock}>
            {selectedJournee.locked ? "Rouvrir journee" : "Fermer journee"}
          </button>

          <button className="admin-btn green" type="button" onClick={saveAll}>
            Sauvegarder
          </button>
        </div>

        <div className="admin-message">
          {message}
        </div>
      </div>

      <div className="section-head">
        <h2>Matchs Ligue 1 - {selectedJournee.title}</h2>
        <span>{matches.length} matchs</span>
      </div>

      <div className="admin-pronos-actions" style={{ marginBottom: 16 }}>
        <button className="admin-btn green" type="button" onClick={addMatch}>
          Ajouter un match Ligue 1
        </button>
      </div>

      <div className="admin-match-grid">
        {matches.map((match, index) => {
          const blocked = match.status === "Bloque";

          return (
            <div key={match.id} className={`admin-match-card ${blocked ? "blocked" : ""}`}>
              <div className="admin-match-top">
                <div className="admin-number">{index + 1}</div>

                <button
                  type="button"
                  className={`admin-status-btn ${blocked ? "blocked" : ""}`}
                  onClick={() => toggleMatchStatus(match.id)}
                >
                  {blocked ? "Bloque" : "Ouvert"}
                </button>
              </div>

              <div className="admin-fields">
                <label>
                  Equipe domicile
                  <input
                    className="admin-input"
                    value={match.home || ""}
                    onChange={(e) => updateMatch(match.id, { home: e.target.value })}
                    placeholder="RC Lens"
                  />
                </label>

                <label>
                  Equipe exterieur
                  <input
                    className="admin-input"
                    value={match.away || ""}
                    onChange={(e) => updateMatch(match.id, { away: e.target.value })}
                    placeholder="OL"
                  />
                </label>

                <div className="admin-two">
                  <label>
                    Date
                    <input
                      className="admin-input"
                      type="date"
                      value={toDateInputValue(match.date)}
                      onChange={(e) => updateMatch(match.id, { date: e.target.value })}
                    />
                  </label>

                  <label>
                    Heure
                    <input
                      className="admin-input"
                      type="time"
                      value={toTimeInputValue(match.time)}
                      onChange={(e) => updateMatch(match.id, { time: e.target.value })}
                    />
                  </label>
                </div>

                <label>
                  Resultat final
                  <div className="score-row">
                    <input
                      className="admin-input"
                      type="number"
                      min="0"
                      value={match.homeScore || ""}
                      onChange={(e) => updateMatch(match.id, { homeScore: e.target.value })}
                      placeholder="0"
                    />
                    <strong>-</strong>
                    <input
                      className="admin-input"
                      type="number"
                      min="0"
                      value={match.awayScore || ""}
                      onChange={(e) => updateMatch(match.id, { awayScore: e.target.value })}
                      placeholder="0"
                    />
                  </div>
                </label>
              </div>

              <div className="admin-card-actions">
                <button
                  type="button"
                  className={match.adminValidated ? "admin-validate-btn validated" : "admin-validate-btn"}
                  onClick={() => validateAdminMatch(match.id)}
                >
                  {match.adminValidated ? "Validé" : "Valider"}
                </button>
              </div>
            </div>
          );
        })}
      </div>

      <div className="section-head">
        <h2>Les 4 matchs bonus - {selectedJournee.title}</h2>
        <span>4 bonus modifiables</span>
      </div>

      <div className="admin-match-grid">
        {bonus.map((match, index) => (
          <div key={match.id} className="admin-match-card bonus">
            <div className="admin-match-top">
              <div className="admin-number">{index + 1}</div>
              <span className="admin-pill">Bonus</span>
            </div>

            <div className="admin-fields">
              <label>
                Competition
                <input
                  className="admin-input"
                  value={match.league || ""}
                  onChange={(e) => updateBonus(match.id, { league: e.target.value })}
                  placeholder="Premier League"
                />
              </label>

              <label>
                Equipe domicile
                <input
                  className="admin-input"
                  value={match.home || ""}
                  onChange={(e) => updateBonus(match.id, { home: e.target.value })}
                  placeholder="Liverpool"
                />
              </label>

              <label>
                Equipe exterieur
                <input
                  className="admin-input"
                  value={match.away || ""}
                  onChange={(e) => updateBonus(match.id, { away: e.target.value })}
                  placeholder="Chelsea"
                />
              </label>

              <div className="admin-two">
                <label>
                  Date
                  <input
                    className="admin-input"
                    type="date"
                    value={toDateInputValue(match.date)}
                    onChange={(e) => updateBonus(match.id, { date: e.target.value })}
                  />
                </label>

                <label>
                  Heure
                  <input
                    className="admin-input"
                    type="time"
                    value={toTimeInputValue(match.time)}
                    onChange={(e) => updateBonus(match.id, { time: e.target.value })}
                  />
                </label>
              </div>

              <label>
                Resultat final bonus
                <div className="score-row">
                  <input
                    className="admin-input"
                    type="number"
                    min="0"
                    value={match.homeScore || ""}
                    onChange={(e) => updateBonus(match.id, { homeScore: e.target.value })}
                    placeholder="0"
                  />
                  <strong>-</strong>
                  <input
                    className="admin-input"
                    type="number"
                    min="0"
                    value={match.awayScore || ""}
                    onChange={(e) => updateBonus(match.id, { awayScore: e.target.value })}
                    placeholder="0"
                  />
                </div>
              </label>
            </div>

            <div className="admin-card-actions">
              <button
                type="button"
                className={match.adminValidated ? "admin-validate-btn validated" : "admin-validate-btn"}
                onClick={() => validateAdminMatch(match.id)}
              >
                {match.adminValidated ? "Validé" : "Valider"}
              </button>

              <button
                className="red"
                type="button"
                onClick={() =>
                  updateBonus(match.id, {
                    league: "",
                    home: "",
                    away: "",
                    date: "",
                    time: "",
                    homeScore: "",
                    awayScore: "",
                    adminValidated: false
                  })
                }
              >
                Effacer
              </button>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}












